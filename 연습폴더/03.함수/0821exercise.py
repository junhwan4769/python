from function2 import *

sale = [
    {'code':1, 'name':'냉장고', 'price':250, 'qnt':5},
    {'code':2, 'name':'세탁기', 'price':150, 'qnt':3},
]

while True:
    menuPrint('상품관리')
    menu = input('메뉴선택>')

    if menu == "0":
        print('프로그램을 종료합니다.')
        break

    elif menu == "1":
        new_code = newCode(sale)
        print(f"상품코드>{new_code}")
        new_name = input('상품이름>')
        if new_name == '': continue
        new_price = input('상품가격>')
        if new_price == '': new_price = 0 
        new_qnt = input('판매수량>')
        if new_qnt == '': new_qnt = 0 
        sale.append({'code':new_code,'name':new_name, 'price':new_price, 'qnt':new_qnt})
        print('등록 완료')

    elif menu == "2":
        code = inputNum('검색 코드>')
        index = search(sale,code)
        if index == None:
            print(f"상품이름:{code}번 상품이 없습니다.")
        else:
            s = sale[index]
            print(f"상품이름:{s['name']}")
            print(f"상품가격:{s['price']:,}만원")
            print(f"판매수량:{s['qnt']}개")
            print(f"총매출값:{s['price']*s['qnt']:,}만원")

    elif menu == "3":
        for s in sale:
            print(s)

    elif menu == "4":
        code = inputNum('삭제할 코드>')
        if code == '': continue
        index = search(sale, code)
        if index == None:
            print(f"상품이름:{code}번 상품이 없습니다.")
        else: 
            s = sale[index]
            print(s)
            sel = input('삭제할거니?(Y)')
            if sel == "Y" or "y":
                sale.pop(index)
                print('삭제완료')

    elif menu == "5":
        code = inputNum('수정할 코드>')
        if code == '': continue 
        index = search(sale, code)
        if index == None:
            print(f"상품이름:{code}번 상품이 없습니다.")
            continue
        s = sale[index]
        new_name = input(f"상품이름:{s['name']}>")
        if new_name == '': new_name = s['name']
        new_price = input(f"상품단가:{s['price']:,}만원>")
        if new_price == '': new_price = s['price']
        new_qnt = input(f"판매수량:{s['qnt']:,}만원>")
        if new_qnt == '': new_qnt = s['qnt']
        sel = input('수정할거니?')
        if sel == "Y" or "y" :
            sale[index] = {'code': code, 'name': new_name,'price': new_price, 'qnt':new_qnt}
            print('수정 완료')


